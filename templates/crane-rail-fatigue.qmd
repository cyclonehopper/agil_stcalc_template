...
execute:
  daemon: 600
  cache: true
  dir: "C:/Users/Josua/QuartoScratch"
...
```
- Review design for fatigue of existing rail
- Evaluate the remaining design life of existing rail, if fatigue damage is less than 1.0
...
- Space Gass for supporting steel work
- RFEM for 3D modelling of rail bar
...
- AS 1418.18 - 2021 Cranes, hoists and winches - Part 18: Crane runways and monorails
- AS 4100    - 1998 Steel structures
- AS 5100.2  - 2017 Bridge Design - Part 2: Design Loads
- AS 5221.1  - 2021 Cranes - Design principles for loads and load combinations
- BS 7608    - 1993 Code of practice for Fatigue design and assessment of steel structures

## Reference Documents

Reference documents used in this design include:

- Existing structure design drawings
...
- Rail bar material for 31kg rail is taken as similar to 41kg rails 
- Fy = 420MPa, Fu = 700MPa per AS 1085.1 
...
- For strain data, +ve is compression
- Stress analysis results from RFEM, +ve is tension

## Calibration of wheel loads

For the assessment of fatigue damage and remaining life, it is important to get a relatively good data. In this case, strain data was deemed important to calibrate the loads. The strain data are used to reverse calculate the actual wheel loads. Statistical analysis is then used to make sense of the data.  For vertical load calibration, the average stress at the mid-thickness of the rail web is used. As will be shown in @sec-strain-data-analysis, the average vertical stresses ($\sigma_z$) appears to have some scatter. The wheel load is then calibrated to achieve an average stress of one standard deviation above the mean.  This correlates well with the load estimated based on design drawings, refer @sec-calculated-wheel-load.

## Design wheel loads

THe design wheel loads will be derived from the calibrated vertical load. The tripper is treated like a rail car, hence the dynamic factors (more conservative than from AS 5221.1, for dead load only) will be taken from AS 5100.  Steering or skewing firces will be derived from AS 5221.1.

## Fatigue assessment

The fatigue assessment is to be carried out using the S-N curves in AS 4100.  However, guidance from BS 7608 are taken into consideration regarding the treatment of the compressive stress component of the stress range. 

The rolling contact fatigue is taken into consideration when verifying the design of the size of the rail and wheel, refer @sec-legacy-design, hence no further check is done on the rail contact.  However, for the critical web stresses due to bending caused by transverse or eccentric load, fatigue analysis is carried in accordance with AS 4100 S-N curve.
...
using MyUnits, MyHelper, WindLoadModule, AS4100, MaterialModule, Distributions
```
...
```{julia}
μ_rolling = 0.003 # typical design values for rolling steel rails to steel wheels
Gtotal = Pwheel_Qoper * 4
...
# Strain Data - Ocean side rail bar {#sec-strain-data-analysis}
...
![RFEM overview model of midjoint, landside, simulation](images/rfem_sim/rfem_model_mj_sim.png){#fig-rfem-overview-midjoint-sim}

![Closeup of rail in @fig-rfem-overview-midjoint-sim](images/rfem_sim/rfem_model_mj_sim-closeup.png)

![RFEM overview model of end joint, landside simulation](images/rfem_sim/rfem_model_ej_sim.png){#fig-rfem-overview-endjoint-sim}
...
![Mid joint stresses, reverse load simulation](images/rfem_sim/Wheel-load-sim-MJ-stresses.png){#fig-wheel_load_sim_mj}

## End Joint stresses results

The average vertical stress at the end joint from strain data is taken as the statistical mean + 1 standard deviation = `{julia} mean_savg_ej + stddev_savg_ej`.  @fig-wheel_load_sim_ej shows the RFEM model simulation of the wheel load at the end joint.

![End joint stresses, reverse load simulation](images/rfem_sim/Wheel-load-sim-EJ-stresses.png){#fig-wheel_load_sim_ej}
...
```{julia}
Pwheel_fatdes = γdyn_fat_des * (Pwheel_Qoper) |> f2
```
...
The load is less than the Gantrail limit of 130MPa, pass.
...
```{julia}
ω = 16m / (π * Dwheel) / minute |> minute^-1
```
...
# Legacy Design per AS1418.1-2002, for purposes of determining remaining life{#sec-legacy-design}
...
tripper_passes = 88000;  # the number of ship loader passes to date 

Nxw = tripper_passes * 2 * 10  # assume design life is 2x to date
CR = 20000 / Nxw^(2 / 3)
PT = CR * PTS |> f2
```

by inspection, the permissible load is `{julia} PT` \> design load of `{julia} Pwheel_fatdes`, pass; even for 10x the current design life.

![Table 7.20.6.4](images/general/table-7-20-6-4.png){width="450"}


# Fatigue Damage Assessment, Existing rail (31 kg/m), from local web bending due to skewing/steering load

For the existing rail, fatigue damage assessment is carried out to determine the current state of the rail and if available, the future life of the rail. Assessment is based on AS4100, and its commentary in conjunction with British Standard BS7608-1993 clause 3.4, where for non-welded member, the portion of the compressive stress in the stress range maybe reduced to 60% of the range from zero to maximum compressive stress to maximum tensile stress. A fatigue damage ratio of less than 1.0 is considered passing, otherwise failed given a particular stress range and number of cycles.
...
function get_srange(;
    ...
    solidnum=1,
    surfnum=1,
    xrange::Union{Tuple{Real,Real},Nothing}=nothing,
...
![Wheel load, eccentric](images/rfem_existing_design/wheel_load_ecc.png)

![Wheel load, Skewing/steering](images/rfem_existing_design/wheel_load_skew.png)
...
# assume the same wheel load for front and back wheels
...
The accumulated fatigue damage is `{julia} res.r_damage` $\le$ 1.0, pass
...
years_to_future_life = 10  # years into the future from 88k passes
Npasses_per_yr_future = 2490  # projected tripper passes per year

cycle1 = tripper_passes + years_to_future_life * Npasses_per_yr_future  # 1st wheel
cycle2 = tripper_passes + years_to_future_life * Npasses_per_yr_future  # 2nd wheel
...
#| 
csv_file_solid_stresses_ej_25 = dir * "\\xlsx\\25071-ST-CAL-0003_A-EJ+25-existing\\DS2 ! Solids ! Basic Stresses.csv"
df_sigma_EJ_25_existing = coalesce.(CSV.read(csv_file_solid_stresses_ej_25, DataFrame; header=2, skipto=3), 0.0)
...
# Δf1, df = get_srange(df_sigma=df_sigma_EJ_25_existing, surfnum=21 )
...
```{julia}
n_cycles_remaining = Int64(1.6tripper_passes)
cycle1 = tripper_passes + n_cycles_remaining  # 1st wheel
cycle2 = tripper_passes + n_cycles_remaining  # 2nd wheel

category = 160  # non-welded category

tp = 24mm # flange plate thickness
ϕ = 1.0  # strength factor, complies all in 11.1.5(a) to (d)
γF = 1.1 # fatigue load factor per AS 4100
# γ_scale_to_end = 198 / 166.0  # per RFEM model simulation
...
Δf1, df = get_srange(df_sigma=df_sigma_mj_existing, surfnum=13)
...
#| output: false
#| echo: false
area = 2 * b_hertz * b_head  # contact patch
Ixx = 2 * b_hertz * b_head^3 / 12

p_no_ecc, pmax, pmin = Pwheel_fatdes / ph.area .+ (Pwheel_fatdes * er_convex * b_head / 2 / Ixx) * [0, 1, -1] .|> MPa
```
...
# PARAMETERS
# ----------------------------------------------------------
σy = fyr   # Pa, yield strength of rail steel (e.g., 900 MPa)
npoints = 201;
```
...
![Base stress, self weight + 0.4Qs](images/general/stress_range_case102.png){width="90%"}

![Stress range, moving loads, fatigue stresses](images/general/stress_range_ML_SC1.png){width="90%"}
...
```{julia}
Δf1 = max(93 + 5.8 - 6.2, 83 + 15.15 - 4.64)MPa # max stress range due to design wheels
Δf2 = Δf1                # 2nd wheel
...
```{julia}
Δf1 = max(62 + 4, 76 - 6)MPa # max stress range due to design wheels
Δf2 = Δf1
...
# Summary of Findings of Fatigue Assessment of Existing 31kg rail

- For the internal section of the rail, the accumulated fatigue damage is about 5%, in other words, for all intents and purposes, it has infinite life remaining.
- For the end section of the rail, the critical section is at the root radius under the head

    - where the fatigue damage has reached 100%.
    - and the design life is reduced to about 114000 cycles

- For the section, where the load is 25mm from the end joint

    - the critical section is the root radius at the foot of the rail
    - and the accumulated fatigue damage is about 38%, which means it still has capacity for another 1.6x life cycles to date.

{{< pagebreak >}}

# Appendix A - Site Informations{-}

{{< pagebreak >}}

```{=latex}
\includepdf[pages=-, landscape]{pdf-to-embed/appendix_a.pdf}
``` 

# Appendix B - Strain Gauge Locations{-}

{{< pagebreak >}}

```{=latex}
\includepdf[pages=-, landscape]{pdf-to-embed/appendix_b_strain_gauge_locations.pdf}
``` 